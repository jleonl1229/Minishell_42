...
**	no functionality for the tilde char (~, shorthand for the home dir path)
*/
...
**	getenv(envvar_name) -->returns NULL if 
**	no env_var found or a pointer to the value
**	chdir(absolute path to directory)--> returns 0 if 
**	successfully changed to directory, -1 otherwise
**	getcwd()--> returns char * (the absolute path of current wd) if 
