...
/*   Updated: 2024/09/06 15:49:53 by mzuloaga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
extern int	g_signal; //global var for signals

//struct created to diff. the return value of cmd_arr() function in parsing
/*
...
**      0: no error, 1: count is 0,
**		2: malloc, close or open failure (terminate program)
*/
...
/*
**	struct of a node in a linked list. 
**	Each node's data made of env var name + content 
**	a copy of char **env, done to easily manipulate env_vars by our program
*/
...
typedef struct s_parsed_data
{
	...
	int						simple_in_redir; //stdin redirection
	t_list					*here_doc; //delimiter to handle here_doc feature
	int						last_fd; //last assigned ouput fd
...
{
	t_env			*env_header; // head of the linked list holding the env vars
	char			**env; //env vars in char ** format. Needed in execve
	char			*prev_line; //last line saved to history
	char			*new_line; //cmd line with spaces around special chars
	t_parsed_data	*parsed_header; //head of l-list with the pipe segments data
	char			*last_exit_status; //holds the exit status of the last cmd
}	t_sh_data;

//execution/execution.c
void			child_out_redir(t_parsed_data *header, int fd[2]);
void			child_process(t_sh_data *sh, t_parsed_data *header, int fd[2]);
void			exec_invalid_infile(t_sh_data *sh,
					int fd[2], t_parsed_data **header);
void			prepare_next_cmd(t_parsed_data *header, int fd[2]);
int				exec_manage_parent(t_sh_data *sh,
					t_parsed_data **header, int fd[2]);
int				exec_reinit_loop(t_sh_data *sh, t_parsed_data **header,
					int fd[2]);
...
int				execute_builtin(int fd[2],
					t_parsed_data **header, t_sh_data *sh);
...
//parsing/parsing.c
char			**split_by_pipe(char *line, int start,
					int segment_index, int i);
void			parse_line(t_sh_data **sh, t_parsed_data **header,
					char **split_space, char **pipe_segments);
...
char			**alloc_get_var_content(char *input, int var_len,
					int start, char **var_name);
char			**get_var_content(char *input, int start, t_sh_data *sh);
char			*act_on_dollar(char *input, int *j, t_sh_data *sh);
char			**env_parse(int single_q, int double_q,
					char **input, t_sh_data *sh);
...
void			simple_export(char **env_var, int set,
					char *arg, t_sh_data *sh);
...
//built_ins/echo.c
int				mini_echo(t_parsed_data *header, int i, int newline);

//built_ins/cd.c
int				chdir_home(t_sh_data *sh, char *old_dir);
int				chdir_oldpwd(t_sh_data *sh, char *old_dir);
int				chdir_basecase(t_parsed_data *header,
					t_sh_data *sh, char *old_dir);
...
void			cd_create(char **env_var, t_sh_data *sh,
					int set, char *execve_var);
void			cd_update(char **env_var, t_sh_data *sh,
					int set, char *execve_var);
...
void			parsing_cleanup(t_sh_data **sh,
					char **pipe_segments, char **split_space);
...
t_result		free_parse_redir(t_result res, int flag,
