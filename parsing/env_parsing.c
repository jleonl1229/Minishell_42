...
/*   Updated: 2024/09/05 19:13:10 by jleon-la         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*expanded_var(char **env_pair, char *input, int *start)
{
	...
	if (env_pair == NULL || *env_pair == NULL)
		return (NULL);
	(*start)++;
	vname_len = ft_strlen(env_pair[0]) + 1;
	if (vname_len == 1 && ft_strlen(input) != 1
		&& ft_strncmp(input, "$", ft_strlen("$")) == 0)
...
/*
**	auxiliary to get_var_content, allocates for the char ** that 
**	will contain the data on the env_var_name and value.
**	Also fills the name of the env_var in var_name
*/
char	**alloc_get_var_content(char *input, int var_len, int start,
	char **var_name)
{
	char	**result;

	while (input[var_len] != '\0'
		&& (ft_isalnum(input[var_len]) == 1 || input[var_len] == '_'
